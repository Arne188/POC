...
object XDetector {
        ...
        for (s in sizes) {
            ...
            while (y + s < bot) {
                ...
                while (x + s < r) {
                    ...
                    if (score >= .47 && (best == null || score > best.score)) {
                        best = Detection(Rect(x, y, x + s, y + s), "X / Schliessen", score + 1.0)
                    }
                    x += step
                }
                y += step
    ...
    private fun findCornerArrow(b: Bitmap, right: Boolean): Detection? {
        val w = b.width
        val h = b.height
        val l = if (right) (w * .78).toInt() else 0
        val r = if (right) w else (w * .22).toInt()
        val bot = (h * .18).toInt()
        var best: Detection? = null
        ...
        for (s in sizes) {
            ...
            while (y + s < bot) {
                var x = l
                while (x + s < r) {
                    val sc = chevronScore(b, x, y, s)
                    if (sc > .50 && (best == null || sc > best.score)) {
                        best = Detection(Rect(x, y, x + s, y + s), "Weiter / Skip", sc + .60)
                    }
                    x += step
                }
                y += step
    ...
    private fun chevronScore(b: Bitmap, x0: Int, y0: Int, s: Int): Double {
        ...
        for (i in -q..q) {
            for (off in -2..2) {
                val x = (cx + abs(i) + off).coerceIn(x0, x0 + s - 1)
                val y = (cy + i).coerceIn(y0, y0 + s - 1)
                diag += lum(b.getPixel(x, y))
                n++
            }
            bg += lum(b.getPixel((x0 + s / 5).coerceAtMost(x0 + s - 1), (cy + i).coerceIn(y0, y0 + s - 1)))
            bn++
        }
        if (n == 0 || bn == 0) return 0.0
        val d = diag / n
        val o = bg / bn
        return ((d - o + 80.0) / 255.0).coerceIn(0.0, 1.0)
    }

    private fun findTopRightAction(b: Bitmap): Detection? {
        val w = b.width
        val h = b.height
        val roi = Rect((w * .55).toInt(), (h * .01).toInt(), (w * .99).toInt(), (h * .14).toInt())
        var bright = 0
        var dark = 0
        var green = 0
        var total = 0
        ...
        for (y in roi.top until roi.bottom step 3) {
            for (x in roi.left until roi.right step 3) {
                ...
                if (ll < 55) dark++
                if (g > 95 && g > rr * 1.15 && g > bl * 1.10) green++
                total++
            }
        }
        ...
        val gr = green.toDouble() / total
        val wide = maxX > minX && (maxX - minX) > w * .09 && (maxY - minY) > h * .012
        if (!((br > .025 && dr > .28 && wide) || (br > .018 && gr > .18 && wide))) return null

        return Detection(
            Rect(
                (minX - w * .035).toInt().coerceAtLeast(roi.left),
                (minY - h * .012).toInt().coerceAtLeast(roi.top),
                (maxX + w * .035).toInt().coerceAtMost(roi.right),
                (maxY + h * .012).toInt().coerceAtMost(roi.bottom)
            ),
            "Store / Install",
            .35
        )
    ...
    private fun findTopLeftNext(b: Bitmap): Detection? {
        val w = b.width
        val h = b.height
        val r = Rect((w * .02).toInt(), (h * .03).toInt(), (w * .32).toInt(), (h * .16).toInt())
        var bright = 0
        var dark = 0
        var total = 0
        for (y in r.top until r.bottom step 4) {
            for (x in r.left until r.right step 4) {
        ...
        val br = bright.toDouble() / total
        val dr = dark.toDouble() / total
        return if (br in .015..0.20 && dr > .35) Detection(r, "Next / Weiter", .65) else null
    }

    private fun xScore(b: Bitmap, x0: Int, y0: Int, s: Int): Double {
        val margin = (s * .16).toInt().coerceAtLeast(2)
        val thickness = (s * .075).toInt().coerceIn(1, 5)
        ...
        if (dn == 0 || bn == 0) return 0.0
        val d = diag / dn
        val o = bg / bn
        if (d < 15) return 0.0
        return (d / (d + o + 1) + (c(x0 + s / 2, y0 + s / 2) / 255) * .10).coerceIn(0.0, 1.0)
    }
