...
class AdWatchService : AccessibilityService() {
    ...
    private var imageHits=0; private var lastImageDetection:XDetector.Detection?=null
    ...
    override fun onAccessibilityEvent(event:AccessibilityEvent?){
        ...
        if(possibleAd){ val close=findLikelyCloseButton(root); if(close!=null){ showNode(close,"Schliessen / Weiter"); alert("Mögliches Schließen-/Weiter-Element erkannt"); return } }
    ...
    private fun finishAd(){ possibleAd=false; imageHits=0; lastImageDetection=null; if(::spotlight.isInitialized)spotlight.hide(); StatsRepository(this).finishAd(System.currentTimeMillis()) }
    ...
    private fun captureAndAnalyze(){ takeScreenshot(Display.DEFAULT_DISPLAY,exec,object:TakeScreenshotCallback{
        override fun onSuccess(result:ScreenshotResult){ if(!Prefs.assistantOn(this@AdWatchService)){result.hardwareBuffer.close();return}; val hb=result.hardwareBuffer
            try{ val hw=Bitmap.wrapHardwareBuffer(hb,result.colorSpace)?:return; val b=hw.copy(Bitmap.Config.ARGB_8888,false); val d=XDetector.findControl(b); b.recycle()
                if(d!=null){ imageHits++; lastImageDetection=d }else{ imageHits=0; lastImageDetection=null }
                val needed=if(possibleAd)2 else 3
                if(imageHits>=needed&&lastImageDetection!=null){ val hit=lastImageDetection!!; startAd(); handler.post{ if(Prefs.overlay(this@AdWatchService)&&::spotlight.isInitialized)spotlight.show(hit.rect,hit.kind); alert("${hit.kind} erkannt") }; imageHits=0 }
            }finally{hb.close()} }
    ...
    private fun findActionElement(n:AccessibilityNodeInfo?):AccessibilityNodeInfo?{ if(n==null)return null; val t=nodeLabel(n); if((n.isClickable||n.isFocusable)&&keywords.any{t.contains(it)})return n; if(n.isClickable&&t in listOf("x","×","✕","✖",">","›","»"))return n; for(i in 0 until n.childCount)findActionElement(n.getChild(i))?.let{return it}; return null }
    ...
    private fun findLikelyCloseButton(n:AccessibilityNodeInfo?):AccessibilityNodeInfo?{if(n==null)return null;if(n.isClickable){val r=Rect();n.getBoundsInScreen(r);val w=resources.displayMetrics.widthPixels;val h=resources.displayMetrics.heightPixels;if(w>0&&h>0&&r.width() in 18..(w*.18).toInt()&&r.height() in 18..(h*.12).toInt()){val top=r.centerY()<h*.24;val side=r.centerX()<w*.22||r.centerX()>w*.78;if(top&&side)return n}};for(i in 0 until n.childCount)findLikelyCloseButton(n.getChild(i))?.let{return it};return null}
